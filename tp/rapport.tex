...
\item Approchons le problème :
  ...
  $\iff \forall 1 <= i <= n, a(\sum_{k=1}^{n}{\alpha_k w_k w_i}) = l(w_i)$

  $\iff \forall 1 <= i <= n, \sum_{k=1}^{n}{a(w_k, w_i) \alpha_i} =
  l(w_i)$
  ...
  \begin{equation*}
    \begin{split}
      ...
      \begin{split}
        ...
        = & (0, ..., 1, ..., 0)
      \end{split} \\
      & \iff w_k(t_j) = \delta_{k,j} \quad 1 <= j <= n-1
    \end{split}
  \end{equation*}
